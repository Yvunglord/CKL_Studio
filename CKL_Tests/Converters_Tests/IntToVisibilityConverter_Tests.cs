...
{
    public class IntToVisibilityConverterTestRunner
    {
        public static TestResult RunAllIntToVisibilityConverterTests()
        {
            var testRunner = new IntToVisibilityConverterTestRunner();
            return testRunner.RunTestsAndPrintReport();
        }

        public TestResult RunTestsAndPrintReport()
        {
            Debug.WriteLine("Запуск тестов IntToVisibilityConverter");
            Debug.WriteLine("");

            var testFixture = new IntToVisibilityConverterTests();
        ...
        {
            return typeof(IntToVisibilityConverterTests)
    ...
    {
        private IntToVisibilityConverter _converter;
        ...
        [Test]
        public void Convert_IntValueMatchesParameter_ReturnsVisible()
        {
            Debug.WriteLine("Тест: Convert_IntValueMatchesParameter_ReturnsVisible");
        ...
        [Test]
        public void Convert_IntValueDoesNotMatchParameter_ReturnsCollapsed()
        {
            Debug.WriteLine("Тест: Convert_IntValueDoesNotMatchParameter_ReturnsCollapsed");

            var result = _converter.Convert(5, typeof(Visibility), "10", _culture);

            Assert.That(result, Is.EqualTo(Visibility.Collapsed));
            Debug.WriteLine("УСПЕХ: значение 5 с параметром '10' возвращает Collapsed");
        }
        ...
        public void Convert_NegativeNumbers_WorksCorrectly()
        {
            ...
            var result1 = _converter.Convert(-5, typeof(Visibility), "-5", _culture);
            var result2 = _converter.Convert(-5, typeof(Visibility), "5", _culture);
        ...
        public void Converter_IsStateless()
        {
            ...
            var result1 = _converter.Convert(5, typeof(Visibility), "5", _culture);
            var result2 = _converter.Convert(5, typeof(Visibility), "5", _culture);
            var result3 = _converter.Convert(5, typeof(Visibility), "10", _culture);
